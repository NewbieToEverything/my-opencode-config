...
- **禁止读取隐藏配置文件** - AGENT 禁止直接读取任何隐藏配置文件（如 `.env`、`.env.*`、`.gitconfig`、`.npmrc` 等），无论任何理由
- **禁止暴露敏感信息** - AGENT 禁止以任何形式暴露、显示、复述或记录隐藏配置文件内容，以及用户名、密码、令牌、API 密钥、邮箱等敏感信息，包括在摘要、代码注释、日志或错误信息中
